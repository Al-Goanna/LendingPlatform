...
goal app call \
    ...
    -f "$LEND_ACCOUNT" \
    --fee 2000 \
    --app-arg "str:partial_pay" \
    --foreign-asset $FOREIGN_ASSET \
    -o partial-pay-noop.tx

# fund payment
goal clerk send \
    -a "$PARTIAL_PAYBACK_PRICE" \
    -t "$APP_ACCOUNT" \
    -f "$LEND_ACCOUNT" \
    -o partial-pay-payment.tx

# group transactions
cat partial-pay-noop.tx partial-pay-payment.tx > list-combined.tx
